...
| D-17 | Governance: `.meta/CLAUDE.md` is the current governance document and `.meta/specs/` is the only spec location. Claude Code writes specs and implements, following `.meta/CLAUDE-CODE-INSTRUCTIONS.md`. Qwen is no longer used: its workflow (`.meta/WORKFLOW.md`), protocol folder (`Agents/Qwen/`) and references are discarded in A5. The Codex protocol documents are superseded. | The instructions have Claude Code implementing every task, and the Qwen workflow points to a spec folder that does not exist. Confirmed by the analyst at the A3 gate. | 2026-09-25 | accepted |
